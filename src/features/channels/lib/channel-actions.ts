...
function updateChannelStatusCache(
  ...
  queryClient.setQueriesData<ChannelListCache>(
    ...
    (oldData) => {
      ...
      let changed = false
      const nextItems = items.map((channel) => {
  ...
  queryClient.setQueryData(channelsQueryKeys.detail(id), (oldData) => {
    const detail = oldData as { data?: { status?: number } } | undefined
    if (!detail?.data) return oldData
...
export async function handleEnableChannel(
  ...
  try {
    ...
    if (response.success) {
      updateChannelStatusCache(queryClient, id, CHANNEL_STATUS.ENABLED)
...
export async function handleDisableChannel(
  ...
  try {
    ...
    if (response.success) {
      updateChannelStatusCache(queryClient, id, CHANNEL_STATUS.MANUAL_DISABLED)
